...
energyTrading = pd.read_csv('지역별_전력거래량_20240502173346.csv')
salesPower = pd.read_csv('행정구역별_용도별_판매전력량_20240501132320.csv')
renewEnergyProduction = pd.read_csv('지역별_신·재생에너지_생산량_비재생폐기물_제외_20240501132022.csv')
renewEnergyGeneration = pd.read_csv('지역별_신·재생에너지_발전량_비재생폐기물_제외_20240501132124.csv')
comsumptPower = pd.read_csv('지역별_부문별_소비실적_20240501180330.csv')
...
rep8 = rep7.replace('-', '-1')
#데이터 타입 설정
"""rep8['연도_datetime'] = pd.to_datetime(rep8['연도'], format='mixed').dt.year
...
# 결측치 처리(5월 7일)
rep9 = rep8.drop('해양 (toe)', axis=1)
rep9.loc[[8,8,8,26,26,26,44,44,44], ['풍력 (toe)', '수력 (toe)', '수열 (toe)']] = 0 #세종 데이터 0으로 대체
rep9.loc[1, '수열 (toe)'] = 18 #서울 데이터 수열 18.0 최빈값으로 대체
rep9.loc[5, '풍력 (toe)'] = 1.5 #광주 데이터 풍력 1.5 평균으로 대체
rep9.loc[[5,23,41], ['수열 (toe)']] = 0 #광주 데이터 수열 0으로 대체
rep9.loc[[3,21,39], ['수열 (toe)']] = 0 #대구 데이터 수열 0으로 대체
rep9.loc[[6,24,42], ['수열 (toe)']] = 0 #대전 데이터 수열 0으로 데체
rep9.loc[[7,25],['수열 (toe)']] = 0 #울산 수열 0으로 대체
#rep9.to_csv('result_RenewEnergyProduction_V2.csv')
#print(rep9.info())
...
reg9 = reg8.replace('-', '-1')
#데이터 타입 설정
reg9['연도_datetime'] = pd.to_datetime(reg9['연도'], format='mixed').dt.year
...
#결측치 처리(5월 7일)
reg10 = reg9.drop(['해양 (MWh)'], axis=1)
reg11 = reg10.drop(['IGCC (MWh)'], axis=1)
reg11.loc[[6,24,42], ['바이오 (MWh)']] = 0
reg11.loc[[8,26,44], ['풍력 (MWh)','수력 (MWh)','바이오 (MWh)']] = 0
#print(reg11.info())
#reg11.to_csv('result_RenewEnergyGeneration_V2.csv')
